  ...
  if (!publicKey) return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-16">
      <div className="text-center mb-10">
        <h1 className="font-display text-3xl font-bold text-[#0F172A] dark:text-[#E2E8F0] mb-3">My Impact</h1>
        <p className="text-[#475569] dark:text-[#94A3B8] font-body">Connect your wallet to see your donation history and impact</p>
      </div>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="font-display text-3xl font-bold text-[#0F172A] dark:text-[#E2E8F0] mb-1">My Impact</h1>
          <div className="flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"/>
            <span className="address-tag">{shortenAddress(publicKey)}</span>
          </div>
        </div>
        <Link href="/projects" className="btn-primary text-sm py-2.5 px-5 flex-shrink-0 gap-2">
            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" /></svg>
            Donate Now</Link>
      </div>

      {dueSubscriptions.length > 0 && (
        <div className="card mb-6 border-[rgba(245,158,11,0.25)] bg-[rgba(245,158,11,0.06)]">
          <h2 className="font-display text-lg font-semibold text-[#B45309] mb-2">Monthly Giving Due Today</h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          <div key={stat.label} className="card text-center shadow-sm border border-[rgba(99,102,241,0.08)] dark:border-[rgba(129,140,248,0.10)]">
            <p className="text-2xl mb-2">{stat.icon}</p>
            <p className="font-display font-bold text-[#0F172A] dark:text-[#E2E8F0] text-lg leading-tight">{loading ? "..." : stat.value}</p>
            <p className="text-xs text-[#64748B] dark:text-[#94A3B8] mt-1 font-body uppercase tracking-wider font-bold opacity-60">{stat.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex border-b border-[rgba(99,102,241,0.10)] dark:border-[rgba(129,140,248,0.12)] mb-6">
        ...
        >
          Saved Projects
          {wishlist.length > 0 && (
            <span className="bg-[rgba(99,102,241,0.08)] dark:bg-[rgba(129,140,248,0.10)] text-[#4F46E5] dark:text-[#818CF8] px-2 py-0.5 rounded-full text-[10px]">{wishlist.length}</span>
          )}
        </button>
      </div>
        ...
        <div className="space-y-8 animate-slide-up">
          ...
          <div className="card">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <h2 className="font-display text-xl font-bold text-[#0F172A] dark:text-[#E2E8F0]">Your Impact Certificate</h2>
            ...
            <div className="card shadow-sm border border-[rgba(99,102,241,0.08)] dark:border-[rgba(129,140,248,0.10)]">
              ...
              <div className="flex flex-wrap gap-4">
                {profile.badges.map((badge, i) => (
                  <div key={i} className="flex items-center gap-3 bg-[rgba(99,102,241,0.04)] dark:bg-[rgba(129,140,248,0.06)] rounded-xl px-4 py-3 border border-[rgba(99,102,241,0.08)] dark:border-[rgba(129,140,248,0.10)] hover:bg-[rgba(99,102,241,0.08)] dark:hover:bg-[rgba(129,140,248,0.10)] transition-colors">
                    <span className="text-3xl">{badgeEmoji(badge.tier)}</span>
                    <div>
                      <p className="font-semibold text-[#0F172A] dark:text-[#E2E8F0] text-sm font-body">{badgeLabel(badge.tier)}</p>
                      <p className="text-[10px] text-[#64748B] dark:text-[#94A3B8] font-body uppercase tracking-widest font-bold opacity-80">Earned {timeAgo(badge.earnedAt)}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="card shadow-sm border border-[rgba(99,102,241,0.08)] dark:border-[rgba(129,140,248,0.10)]">
            ...
            {loading ? (
              <div className="space-y-3">
                {[1,2,3].map(i => <div key={i} className="h-16 bg-[rgba(99,102,241,0.04)] dark:bg-[rgba(129,140,248,0.06)] rounded-xl animate-pulse"/>)}
              </div>
            ) : donations.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-4xl mb-3">🌱</p>
                <p className="text-[#475569] dark:text-[#94A3B8] mb-4 font-body">No donations yet</p>
                <Link href="/projects" className="btn-primary text-sm">Browse Projects →</Link>
              </div>
            ) : (
              <div className="space-y-2">
                {donations.map(d => (
                  <div key={d.id} className="flex items-center gap-4 p-4 rounded-xl bg-[rgba(99,102,241,0.04)] dark:bg-[rgba(129,140,248,0.06)] hover:bg-[rgba(99,102,241,0.08)] dark:hover:bg-[rgba(129,140,248,0.10)] transition-colors border border-transparent hover:border-[rgba(99,102,241,0.10)] dark:hover:border-[rgba(129,140,248,0.12)]">
                    <div className="w-10 h-10 rounded-full bg-[rgba(99,102,241,0.10)] dark:bg-[rgba(129,140,248,0.12)] flex items-center justify-center text-lg flex-shrink-0">🌱</div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-[#0F172A] dark:text-[#E2E8F0] font-body">Project donation</p>
                      {d.message && <p className="text-xs text-[#475569] dark:text-[#94A3B8] italic font-body truncate">&quot;{d.message}&quot;</p>}
                      <p className="text-[10px] text-[#64748B] dark:text-[#94A3B8] font-body uppercase tracking-wider font-bold opacity-70">{timeAgo(d.createdAt)}</p>
                    </div>
                    <div className="text-right flex-shrink-0">
                      ...
                      <a href={explorerUrl(d.transactionHash)} target="_blank" rel="noopener noreferrer"
                        className="text-[10px] text-[#4F46E5] dark:text-[#818CF8] hover:text-[#6366F1] dark:hover:text-[#A5B4FC] font-bold uppercase tracking-widest transition-colors">View tx ↗</a>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="animate-slide-up">
          {savedProjects.length === 0 ? (
            <div className="card text-center py-20">
              ...
              <h2 className="text-xl font-display font-bold text-[#0F172A] dark:text-[#E2E8F0] mb-2">No saved projects yet</h2>
              <p className="text-[#475569] dark:text-[#94A3B8] mb-8 font-body">Save projects you&apos;re interested in to track their progress.</p>
